...
async function main() {
    console.log("🧪 Testing Credit Line on Base Mainnet (Tiny Amount: 0.001 WETH)...\n");
    ...
    if (ethBalance < ethers.parseEther("0.005")) {
        console.log("❌ Insufficient ETH for testing (need at least 0.005 ETH for gas + WETH)");
        return;
    }
    ...
    // Get some WETH for testing - VERY SMALL AMOUNT
    console.log("💸 Getting tiny amount of WETH for testing...");
    ...
    let testAmount = ethers.parseEther("0.001"); // TINY AMOUNT: 0.001 WETH
    if (wethBalance < testAmount) {
        console.log("Wrapping tiny amount of ETH to WETH (0.001 ETH)...");
    ...
    console.log("✅ Transferred 0.001 WETH to factory");

    console.log("⏳ Pausing 5 seconds...");
    await pause(5000);

    // Create a test credit line with TINY amounts
    console.log("🎯 Creating test credit line with TINY REAL Aerodrome integration...");
    const creditLimit = ethers.parseEther("1000"); // Much smaller credit limit
    const apy = 500; // 5% APY
    const initialLiquidity = ethers.parseEther("0.001"); // TINY: 0.001 WETH
    ...
    console.log("- Estimated Cost: ~$0.10 USD (very low risk)");

    try {
        const createTx = await creditLineFactory.createCreditLine(
            "Tiny Test Credit Line",
            "TTCL",
            WETH,
            creditLimit,
            apy,
        ...
        if (creditLineCreatedEvent) {
            ...
            await testTinyCreditLineFunctionality(creditLineAddress, aerodromeAdapterAddress, factoryAddress, deployer);
...
async function testTinyCreditLineFunctionality(creditLineAddress: string, aerodromeAdapterAddress: string, factoryAddress: string, deployer: any) {
    console.log("\n🔍 Testing Tiny Real Credit Line Functionality...");
    ...
    console.log("\n✅ Tiny real credit line functionality test complete!");
    console.log("🎉 SUCCESS: Credit line system works with real Aerodrome contracts!");
    console.log("💰 Total cost was minimal (~0.001 WETH + gas)");
}
